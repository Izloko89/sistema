...
try{
	...
	$sqlEveCheck="SELECT 
		*
	FROM eventos
	WHERE eventos.id_empresa=$empresaid AND fechaevento BETWEEN '$inicioMes' AND '$finMes';";
...
<div id="tabs">
  ...
  <div id="entradas">
  	<form id="entradas_form" class="formularios">
        ...
        <label class="">Cantidad:</label><input type="text" class="cantidad" name="cantidad" /><br>
		<textarea name="obs" id="obs" placeholder="Notas" cols="80" rows="5" style="resize:none;"></textarea><br>
            ...
            <div class="checklist">
				...
				foreach($eventos["salidas"] as $ind=>$v){
					...
					$sql = 'select 
								...
								s.id_articulo,
								f.nombre as familia,
								u.nombre as subfamilia,
								a.nombre as articulo,
								s.cantidad,
								i.cantidad as cnt
							from almacen_salidas s
							INNER JOIN articulos a ON a.id_articulo = s.id_articulo
							INNER JOIN familias f ON f.id_familia = a.familia
							INNER JOIN areas r ON r.id_area= a.area
							INNER JOIN subfamilias u ON u.id_subfamilia = a.subfamilia
							INNER JOIN almacen_inventario i ON i.id_articulo = s.id_articulo
							WHERE id_evento=' . $idEve[$ind - 1];
							$res = $bd->query($sql);
