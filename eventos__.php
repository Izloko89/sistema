    ...
    <h3 class='titulo_form'>Observaciones</h3>
      <form action="scripts/nota_venta_pdf.php" target="_blank">
        ...
        <input type="submit" onclick="this.form.action='scripts/nota_venta_pdf.php'" value="Hoja de bulto" class="flota_der2" />
		  <input type="submit" onclick="this.form.action='scripts/pdf_contrato.php'" value="Contrato" class="flota_der" />
      </form>
    </div>
  </div>
  ...
  <tr class="barra_accion">
	...
	try{
		...
		foreach($cot as $folio=>$d){
			...
			echo '<td><img src="img/check.png" data-cve="'.$folio.'" height="20" onclick="autorizarEve('.$folio.','.$d["clave"].')" /><img class="accion" src="img/edit.png" data-cve="'.$d["clave"].'" onclick="editar(this);" height="20" /><img class="accion eliminar" src="img/cruz.png" data-cve="'.$folio.'" height="20" onclick="eliminar_eve(' . $d["id_evento"] . ',' . $cont . ')"/></td>';
			echo '</tr>';
			$cont++;
		}
		$bd=NULL;
