...
$("#salon").change(function(e) {
	...
	$.ajax({
		url:'scripts/direcion_salon.php',
		cache:false,
		async:false,
		type:'POST',
		data:{
		...
		success: function(r){
			if(r){
				document.getElementById("nombreEvento").value = r;
			}else{
			}
		}
	});
...
function editar(e){
	...
	var check=setInterval(function(){checar_salon()},1000);
}

function checar_salon()
{
	$("#nombreEvento").val("");
	
	if($("#eventocheck").is(':checked'))
	{
	...
	$.ajax({
	  ...
	  success: function(r){
		...
		if(r.bool){
			...
			document.getElementById("imp_cot_m").setAttribute("href", "scripts/pdf_cotizacion_m.php?id=" + r.id_cotizacion);
