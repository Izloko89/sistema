...
try{
	...
	if($art!=""){//si es articulo
		...
		$perece=$res[0]["perece"];
			...
			$sql1="SELECT cantidad FROM almacen_inventario WHERE id_articulo=$art;";
		$res1=$bd->query($sql1);
		...
		if($id_item!=""){//si ya está guardado previamente hay que restar de salidas y entradas para volverlos a escribir
			...
			$idEve = $emp . '_' . $eve;
			$sql = "select total from eventos_total where id_evento = '$idEve'";
			$res = $bd->query($sql);
			$res = $res->fetchAll(PDO::FETCH_ASSOC);
			$tTotal = $res[0]['total'];
			$total = $total + $tTotal;
			$sql = "update eventos_total set total = $total where id_evento = '$idEve'";
			$bd->query($sql);
			
			$cantidadAlmacen = $cantidadexistencia - $cant;
			if($cantidadAlmacen <= 0)
			{
				$cantidadAlmacen = 0;
			}
			if($cantidadexistencia < $cant)
			{
			$sql="insert into almacen_salidas (id_empresa,id_evento,id_articulo,cantidad,fechamontaje) values(1,$eve,$art,'$cantidadexistencia','$montaje')";
			$bd->query($sql);	
				
			}
			else
				{
			$sql="insert into almacen_salidas (id_empresa,id_evento,id_articulo,cantidad,fechamontaje) values(1,$eve,$art,'$cant','$montaje')";
			$bd->query($sql);	
				
			}
			$r["info"]="<strong>Articulo</strong> Agregado";
		}
		...
		if($cantidadexistencia < $cant) // si cantidad es menor a la cantidad solicitada se levanta orden de compra
		{
				//agregamos un nuevo elemento a compras
				$sql="insert into compras (id_empresa,id_Evento,folio,estatus) values (1,$eve,$art,1)";
				$bd->query($sql);
				...
				$sql="insert into compras_articulos (id_compra,id_empresa,id_articulo,cantidad, precio) values ($idcomprafinal,1,$art,$cantFinal, $precio)";
				$bd->query($sql);
